...
struct ScalingStatValuesEntry
{
    ...
    // per-subclass model; implemented in game/DataStores/DBCStores.cpp (not inline here) since
    // it switches on InventoryType/ItemSubclassWeapon, which are game-layer enums this shared
    // header can't depend on. Ported from TrinityCore-Cata (Shauren, "Core/Items: Updated
    // scaling item stat calculations").
    [[nodiscard]] uint32 GetStatMultiplier(uint32 inventoryType) const;
    [[nodiscard]] uint32 GetArmor(uint32 inventoryType, uint32 armorType) const;
    [[nodiscard]] uint32 GetDPSAndDamageMultiplier(uint32 subClass, bool isCasterWeapon, float* damageMultiplier) const;
};
...
{
    uint32  ID;                                             // 0        m_ID
    uint8   Color[3];                                       // 1-3      m_lt_operandType[3]
    //uint32  unk1;                                         // 4
    //uint32  unk2[6];                                      // 5-10
    uint8   Comparator[3];                                  // 11-13    m_operator[3]
    //uint8   unk3[2];                                      // 14-15
    uint8   CompareColor[3];                                // 16-18    m_rt_operandType[3]
    //uint32  unk4;                                         // 19
    uint32  Value[3];                                       // 20-22    m_rt_operand[3]
    //uint32  unk5[2];                                      // 23-24
    //uint8   unk6[6];                                      // 25-30
};
